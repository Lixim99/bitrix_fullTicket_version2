...
<?endif;?>
<?$this->addExternalJS(SITE_TEMPLATE_PATH . '/components/bitrix/news/js/some_ajax.js')?>
<?foreach($arResult["ITEMS"] as $arItem):?>
	...
	?>
	<div id='hidden_answ' style="display:none;"></div>
	<div class="news-item" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
		...
		<?endif;?>
		<a class='ajax_sender' href='#'><?='Получить через Ajax'?></a></br>
		<a href='?news_id=<?=$arItem['ID']?>'><?='Получить через Get'?></a>
		<script>
			flag = 1;
			$('.ajax_sender').click(function (){
				if(flag){
					flag = 0;
					$.ajax({
						url:"<?=$APPLICATION->GetCurUri()?>?news_id=<?=$arItem['ID']?>&ajax=Y",
